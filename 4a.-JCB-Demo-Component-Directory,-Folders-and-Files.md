...
A file that is very important is the alias file at \comdemo\libraries\classmap.php when writing your own components. It has the locations of class files whose names spaces will eventually be obsolete starting with Joomla!5 and are now deprecated and will log warnings in server log files to allow cleaning them up eventually. For now, it provides the path to the right of files that have the old namespace and to the left the new one. Joomla! developers are using the namespace extensively and this conforms the classes to the new ones that should be used in your components sooner rather than later.
